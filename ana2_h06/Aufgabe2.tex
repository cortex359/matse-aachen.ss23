...
\begin{center}
    \begin{tikzpicture}[scale=1]
        \begin{axis}[
                ...
                xticklabels={$-\frac{3}{2}\pi$, $-\pi$, $-\frac{\pi}{2}$, $0$, $\frac{\pi}{2}$, $\pi$, $\frac{3}{2}\pi$},
                xlabel = $x$,
                ylabel = $y$,
                legend cell align={left},
                samples=100,
                domain=-6:6, y domain=-5:6,
            ]

            \addplot [name path=f, red]  {5- 5/(pi^2)*x^2};
            \addplot [name path=g, blue] {4*cos(deg(x/2))};
            \legend{$f(x)$, $g(x)$}
            ...
            \addplot[only marks, mark={asterisk}, mark options={scale=1, fill=red, fill opacity=0.15}] coordinates {(0,3.3890224110311)} node[pin=300:{$(x_s, y_s)$}]{};
        \end{axis}
    \end{tikzpicture}
\end{center}
